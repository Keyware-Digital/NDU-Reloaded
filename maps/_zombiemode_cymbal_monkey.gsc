// For the cymbal monkey weapon, reworked for NDR
...
player_give_cymbal_monkey()
{
	...
	self thread maps\_sounds::player_vox_helper( maps\_sounds::great_weapon_sound, "weapon_vox_done" );
}
	...
	while( true )
	{
		...
		if( IsDefined( grenade ) )
		{
			...
			if( isDefined( grenade ) )
			{
				...
				// Check if the grenade is in a valid zone or playable area - Nacht style
                ...
                {
                    // if monkey fails, fizzle like a Samantha doll and delete
                    grenade.script_noteworthy = undefined;
                    grenade.poi_active = false;
                    grenade.attract_to_origin = false;

                    self thread maps\_sounds::samantha_fail_sound();

                    if( isDefined( model ) )
                    {
                        playFX( level._effect["raygun_impact"], model.origin );
						model thread maps\_sounds::samantha_disappear_sound();
                        model delete();
                    }
...
monitor_zombie_groans()
{
    ...
    while( true ) 
    {
        ...
        {
            // Slot can be empty after a zombie dies or a bad attractor trace
            if( !isDefined( self.attractor_array[i] ) )
            {
                continue;
            }

            // Dead AI still sitting in the array has no usable origin
            if( !isAlive( self.attractor_array[i] ) )
            {
                continue;
            }

            // Weird geo / deleted ent — .origin is undefined and distanceSquared() errors
            if( !isDefined( self.attractor_array[i].origin ) )
            {
                continue;
            }
